...
'''
DF RETURNED:
rid   wordID        word                oracion  meaningID   significado  contextID                                           Contexto combinationID
...
def get_significados_sin_peso(row):
    signif = []
    for lista_signif in row.significado.split(','):
        signif.append(lista_signif.split(';')[0].lower())
    return signif
...
def get_pesos(row):
    ...
    for lista_signif in row.significado.split(','):
        ...
        signif = lista_signif.split(';')[1]
        assert(float(signif) - int(signif) == 0)
        pesos.append(int(signif))
    return pesos
...
def reordeno_matriz(sesgos):
    ...
    '''
    titulos =["wordID", "meaningID", "sesgoBase", "sesgoGen"]
    ...
    for nlayer in range(13):
        ...
        for CID1, sesgoCID1, CID2, sesgoCID2, CID3, sesgoCID3, CID4, sesgoCID4 in bloques:
            ...
            df.append([wordID, 1, sesgoCID1[nlayer], sesgoCID2[nlayer]])
            df.append([wordID, 2, sesgoCID3[nlayer], sesgoCID4[nlayer]])
        '''
        tengo
...
def getBias_withPreparedDf(df, layers, m, model, tokenizer):
    ...
    lista_de_df = reordeno_matriz(sesgos_de_capas)
    return lista_de_df
